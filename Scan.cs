...
namespace Borium.RDP
{
	internal class Scan
	{
		...
		internal static void memcpy(ScanData to, ScanData from)
		{
			...
			to.last_hash[0] = from.last_hash[0];
		...
		internal static void scan_()
		{
			...
			do
			{
				...
				else
				{
					...
					else
					{
						...
						if (text_top == start + 1)
						{
							...
							text_message(TEXT_ERROR_ECHO, $"Unexpected character 0x{ch:X4} \'"
				...
				switch (text_scan_data.extended)
				{
					...
					case SCAN_P_COMMENT:
						if (text_scan_data.extended == SCAN_P_COMMENT_NEST || text_scan_data.extended == SCAN_P_COMMENT_NEST_VISIBLE)
						{
							nestable = true;
						}
