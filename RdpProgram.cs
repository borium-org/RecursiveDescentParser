...
{
	internal class RdpProgram
	{
		internal static string RDP_STAMP
		{
			get
			{
				string value = "Generated on 9/19/2015 11:45:00 AM";
				DateTime buildDate = BuildInfo.GetLinkerTimestampLocal(Assembly.GetExecutingAssembly());
				value += $" and compiled on {buildDate}";
				return value;
			}
		}
#if false
...
private static RdpTreeNodeData rdp_tree_last_child;
		...
		public static void Main(string[] args)
		{
			...
			bool[] rdp_lexicalise = { false }; // print lexicalised output flag

			int[] rdp_textsize = { 1000000 }; // size of scanner text array
	...
	RdpTreeNodeData rdp_tree_root = null;
			...
			arg_numeric('t', "Tab expansion width (default " + rdp_tabwidth[0] + ")", rdp_tabwidth);
			arg_numeric('T', "Text buffer size in bytes for scanner (default " + rdp_textsize[0] + ")", rdp_textsize);
...
private static RdpTreeNodeData rdp_add_child(string id, RdpTreeNodeData rdp_tree)
{
	if (rdp_tree_update)
	{
		...
		new RdpTreeEdgeData(1).insertEdgeAfterconst(rdp_tree_last_child, rdp_tree);
